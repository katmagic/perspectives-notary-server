...
var colors = [ "blue","purple","yellow","orange","cyan","green", "red", "brown" ]; 

function num_sort_desc(a,b) { 
	return b - a; 
} 

 
function setup_color_info(server_result_list,cutoff,color_info) { 
	var key_to_ts_list = {}; 
	...
	for(key in key_to_ts_list) { 
		key_to_ts_list[key].sort(num_sort_desc);
	...
	d_print("main","most_recent_list");
	d_print("main", most_recent_list);
	var color_count = 0; ; 
	for(var i = 0; i < most_recent_list.length && 
			 i < colors.length; i++) { 
		color_count++; 
		color_info[most_recent_list[i].key] = colors[i]; 
	}  	
	return color_count;    
} 
	...
	for(var i = 0; i < server_result_list.length; i++) {
	   ...
	   for(var j = 0; j < results.obs.length; j++) { 
		...
		for(k = 0; k < obs.timestamps.length; k++) { 
			...
			var x_cord = x_offset + 
				parseInt(pixels_per_day * SEC2DAY(time_since)); 
			var span_width = pixels_per_day * SEC2DAY(duration); 
