	...
	case HOST_OK:
                if(options.notary_always_probe) {
                        do_probe_check(hostname, port, host_key, original_real_uid,
                                options.notary_always_prompt,NULL);
                }
		...
		break;
	case HOST_NEW:
                do_probe_check(hostname, port, host_key, original_real_uid,1,NULL); 
		...
		break;
	case HOST_CHANGED:
                do_probe_check(hostname, port, host_key, original_real_uid,1,file_key); 
