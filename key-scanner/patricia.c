...
New_Patricia (int maxbits)
{
    patricia_tree_t *patricia = 
      (patricia_tree_t*)calloc(1, sizeof *patricia);
...
void add_prefix(patricia_tree_t *tree, char *prefix_str){
	prefix_t *prefix;
	prefix = ascii2prefix(AF_INET, prefix_str);
	patricia_lookup(tree, prefix); // this adds it to tree
	Deref_Prefix(prefix);
}
...
// return 1 if IP is in the list
// otherwise 0
char ip_in_trie(patricia_tree_t *tree, uint32_t ip_addr) {
	prefix_t prefix;
	prefix.family = AF_INET;
	prefix.bitlen = 32;
	memcpy(&prefix.add.sin, &ip_addr, 4);
	return (patricia_search_best(tree, &prefix) != NULL);
}
...
main(void)
{
    ...
    load_db_to_trie(tree, "exceptions");

    uint32_t ip1, ip2;
    inet_aton("10.1.1.0", (struct in_addr*)&ip1);
    inet_aton("9.9.1.1", (struct in_addr*)&ip2);

    if(ip_in_trie(tree, ip1))
	printf("correct\n");
    else
	printf("wrong\n");

    if(ip_in_trie(tree, ip2))
	printf("wrong\n");
    else
	printf("correct\n");
