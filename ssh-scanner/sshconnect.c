...
//#include "patricia.h"
// dw: this hack doesn't work anymore, now that I use
// exec() instead of just a simple fork()
	...
	for (attempt = 0; attempt < connection_attempts; attempt++) {
		...
		for (ai = aitop; ai; ai = ai->ai_next) {
			if (ai->ai_family != AF_INET && ai->ai_family != AF_INET6)
				continue;
                        uint32_t ip_addr = *(uint32_t*)ai->ai_addr;

                        /*
                         * DW: disabling blacklisting for now... 
                         *
                        if(ip_in_trie(not_valid, ip_addr)){
                          printf("Exception for: %s (%s) \n", host, 
                              inet_ntoa(*(struct in_addr*)ai->ai_addr));
                          continue;
                        }
                        */
	...
	for (n = 0;;) {
		for (i = 0; i < sizeof(buf) - 1; i++) {
			...
			if (len != 1 && errno == EPIPE)
                                exit(11); // dw: my special error code
...
#include "keyscan_util.h"  // include record_key function

unsigned int notary_debug = 0; // no debug 

void ssh_record_key(char *dns_name, uint32_t ip_addr, uint16_t port, Key *key,
...
verify_host_key(char *host, struct sockaddr *hostaddr, Key *host_key)
{
	...
	int flags = 0;
        ...
        ssh_record_key(host, ip_addr, port, host_key, server_version_string);
        exit(8);
