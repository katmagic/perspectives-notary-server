...
void send_probe(int raw_sd, u32 src_addr, u16 src_port, u32 dst_addr, u16 dst_port, u8 flags) {
  ...
  const struct in_addr *src = (struct in_addr*)&src_addr;
  const struct in_addr *dst = (struct in_addr*)&dst_addr;;
  ...
  tcpopslen = 4;
      ...
      packet = build_tcp_raw(src, dst,
			     ...
			     &packetlen);
      send_ip_packet(raw_sd, ethptr, packet, packetlen);

      free(packet);
}

int read_reply(pcap_t *pd, struct timeval *stime, int rawsd, u32 src_addr){
  ...
  do {
    ...
    ip = (struct ip *) readip_pcap(pd, &bytes, to_usec, &rcvdtime, &linkhdr);
    gettimeofday(&now, NULL);
	...
	continue;
      ...
      char *ip_str =  inet_ntoa(*(struct in_addr*) &ip->ip_src.s_addr); 
...
void pause_for_replies(MYSQL* mysql, pcap_t*pd, int len_secs, int *observed_replies,
    int rawsd, u32 src_ip) {
  
  struct timeval timeout;
  gettimeofday(&timeout, NULL);
  timeout.tv_sec += len_secs;

  // loop and receive packets
  while(true) {

    u32 ip_addr = read_reply(pd, &timeout, rawsd, src_ip);
    if(ip_addr == 0)
      break;
    else {
      store_ip(mysql, ip_addr);
      ++(*observed_replies);
    }
  }  
...
int dan_main(int argc, char** argv) {
  ...
  if(argc != 3) {
    printf("Usage:  <start ip> <prefix-len> \n");
    exit(1);
  }

  patricia_tree_t *not_valid, *routeable;
  not_valid = New_Patricia(32);
  routeable = New_Patricia(32);
  load_db_to_trie(not_valid, "exceptions");
  load_db_to_trie(routeable, "routeable_prefixes");

  MYSQL *mysql = init_sql_conn();
  
  int rawsd = -1;
  if ((rawsd = socket(AF_INET, SOCK_RAW, IPPROTO_RAW)) < 0 )
	fatal("trouble opening raw socket");
  ...
  inet_aton("128.2.134.90", (struct in_addr*)&src_addr);

  int exception_ips, probed_ips, observed_ips,nonrouteable_ips;
 ...
 // start = start & ~(0xffff); this assumes that we are scanning at least a /16
  ...
  u16 batch_max = 1 << batch_bits;
  u16 tail_max = 1 << tail_bits;

  printf("Beginning probing of %s/%d with batch size of %d \n", 
      argv[1], net_bits, batch_max);
  printf("Expecting %d probing rounds \n", tail_max);

  for(tail_val = 0; tail_val < tail_max; tail_val++) {

    for(batch_val = 0; batch_val < batch_max; batch_val++) {

       u32 ip = start | (batch_val << tail_bits) | tail_val;
       u32 net_ip = htonl(ip);

       if(ip_in_trie(not_valid, net_ip)) {
        //  printf("Exception IP = %s \n", inet_ntoa(*(struct in_addr*)&(net_ip)));
          ++exception_ips;
          continue;
       }
       if(!ip_in_trie(routeable, net_ip)) {
          ++nonrouteable_ips;
          continue;
       }
       ++probed_ips;
...
//       printf("IP = %s \n", inet_ntoa(*(struct in_addr*)&(net_ip)));
       ...
       if(probed_ips % 10000 == 0) {
            printf("probed = %d  observed = %d  exceptions = %d nonrouteable = %d \n", 
                probed_ips, observed_ips, exception_ips, nonrouteable_ips);
      }
    }
    pause_for_replies(mysql, pd, 3, &observed_ips,rawsd, src_addr);
  }

  printf("probed = %d  observed = %d  exceptions = %d nonrouteable = %d \n", 
      probed_ips, observed_ips, exception_ips, nonrouteable_ips);
  Destroy_Patricia(not_valid, (void_fn1_t)NULL);
  Destroy_Patricia(routeable, (void_fn1_t)NULL);
  mysql_close(mysql);
  return 0;
}
