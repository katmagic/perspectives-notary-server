...
void queue_popfront(flex_queue *q, void *data_out) {
  ...
  int total_size = q->max_elems * q->elem_size;
  int front_offset = q->first_elem - q->data;  
  float frac = ((float)front_offset) / ((float) total_size);
  if(front_offset > 10 && frac > 0.1) {
    // thins are unbalanced enough, let's shift
