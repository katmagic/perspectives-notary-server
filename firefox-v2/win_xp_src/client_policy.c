...
#include "gettimeofday.h" // WIN32 compat function
...
ssh_key_info* find_key_at_time(ssh_key_info_list *notary_obs, uint32_t time) {
    ...
    list_for_each(inner_pos,&(notary_obs->list)) {
      ...
      elem = list_entry(inner_pos, ssh_key_info_list, list);
      info = elem->info;
      timespans = FIND_TIMESPANS(info);
      num_spans = ntohs(info->num_timespans); 
      for(i = 0; i < num_spans * 2; i = i + 2) {
...
uint32_t find_oldest_most_recent(SSHNotary *notary, 
  ...
  list_for_each(outer_pos,&notary->notary_servers.list){
    ...
    most_recent = find_most_recent(server); 
...
int get_num_valid_notaries (SSHNotary *notary, uint32_t cur_time, 
  ...
  list_for_each(outer_pos,&notary->notary_servers.list){
    ...
    cur_key = find_key_at_time(server->notary_results, most_recent);
    is_match = test_key_match(cur_key,key_data,key_len,key_type); 
    if(is_match) {
...
BOOL has_quorum_at_time(SSHNotary *notary, char *key_data, uint16_t key_len, 
  ...
  list_for_each(outer_pos,&notary->notary_servers.list){
    ...
    cur_key = find_key_at_time(server->notary_results, time);
...
uint32_t get_quorum_duration(SSHNotary *notary, char *key_data, 
    ...
    uint32_t stale_limit_sec, BOOL *is_cur_consistent) {
		BOOL consistent, nonzero_duration;
		int size, i,diff; 
		flex_queue *time_changes; 
		uint32_t oldest_most_recent, test_time; 
        struct timeval now;
        gettimeofday(&now, NULL);


        consistent = check_current_consistency(notary, quorum_size, now.tv_sec, 
        ...
        *is_cur_consistent = TRUE; 
        ...
        time_changes = queue_init(20, sizeof(int)); 
        get_all_time_changes(notary, time_changes); 
        
        oldest_most_recent = find_oldest_most_recent(notary, 
        ...
        size = queue_size(time_changes); 
        
        nonzero_duration = FALSE; 
        queue_sort(time_changes, uint_compare); 
        test_time = now.tv_sec; 
        for(i = 0; i < size; i++) {
...
uint32_t most_recent_sighting(SSHNotary *notary, char *key_data, 
	...
	list_for_each(outer_pos,&notary->notary_servers.list){
        ...
        if(server->notary_results == NULL) {
			...
			timespans = FIND_TIMESPANS(info);
            for(i = 0; i < info->num_timespans * 2; i = i + 2) {
				uint32_t end = timespans[i + 1];
                if(end > most_recent_end) {
					most_recent_end = end;
                }
